...
class SalePaymentsModel extends DbConfig
{
    ...
    public function getTotals($stime, $etime, $status=null, $statparity= true, $groupmethod=false, $ttype=null){
        ...
        $placeholders = [":stime"=>$stime, ":etime"=>$etime];
        $methodSel = $groupmethod ? "p.method as method, " : "";
        $sql = "SELECT ${methodSel}COALESCE(SUM(p.amount), 0) as stotal, COUNT(p.id) as snum, COALESCE(GROUP_CONCAT(s.ref SEPARATOR ','),'') as refs FROM sale_payments as p LEFT JOIN sales as s ON p.saleid=s.id WHERE (s.processdt>= :stime AND s.processdt<= :etime)";
